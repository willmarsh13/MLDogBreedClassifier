...
inspection_bottleneck = InceptionV3(weights='imagenet', include_top=False, input_shape=(224, 224, 3))

# Freeze weights to not update during training to avoid overfitting
for layer in inspection_bottleneck.layers:
    layer.trainable = False

# Add classification layers and prevent overfitting
model = Sequential([
    inspection_bottleneck,
...
checkpointer = ModelCheckpoint(filepath='saved_model_Inception.h5', verbose=1, save_best_only=True)
...
model.load_weights('saved_model_Inception.h5')
